...
export type LogoutAllUseCase = {
  execute(input: LogoutAllInput): Promise<void>;
};

export type LogoutAllUseCaseDeps = {
  db: Kysely<DatabaseSchema>;
  sessionService: SessionService;
  membershipReader: MembershipReader;
};

export const createLogoutAllUseCase = (deps: LogoutAllUseCaseDeps): LogoutAllUseCase => {
  const auditWriter = createAuditWriter();

  return {
    async execute(input: LogoutAllInput): Promise<void> {
      const { userId } = await deps.sessionService.validateSession(input.token);
      ...
      await runInTransaction(deps.db, async (trx) => {
        ...
        for (const tenantId of tenantIds) {
          await auditWriter.write(trx, {
            ...
            targetType: 'auth_session_collection',
            targetId: userId,
            after: { revoked_all: true },
