...
export type RollbackConfigOutput = {
  configId: string;
  activeConfigId: string;
  previousConfigId: string | null;
};

export type RollbackConfigUseCase = {
  execute(input: RollbackConfigInput): Promise<RollbackConfigOutput>;
};

export type RollbackConfigUseCaseDeps = {
      ...
      return runInTransaction(deps.db, async (trx) => {
        ...
        if (!validationReport.isValid) {
          throw new PublishingError({
            ...
            message: 'Config cannot be rolled back because validation failed',
            details: {
              tenant_id: input.tenantId,
              config_id: input.configId,
